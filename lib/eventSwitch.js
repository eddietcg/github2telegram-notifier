...
module.exports.eventSwitch = async function (gh_event, body) {
    ...
    switch (gh_event) {
        ...
        case 'push': {
            ...
            const author_name = (head_commit && head_commit.author) ? head_commit.author.name : sender.login

            let commits_str = ""
            for (commit of commits) {
                const msg = escape(commit.message);
                commits_str += `<a href='${commit.url}'>View Details 🔗</a>  ${msg}\n`
            }

            return msg + `[${cl(author_name)}](${sender.html_url})` +
